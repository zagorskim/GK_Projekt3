...
import { canvas1context, canvas1Data, canvas2context, canvas2Data, canvas3context, canvas3Data, canvas4context, canvas4Data } from "../state/CanvasState";
...
export default function ControlPanel() {
    ...
    function loadFile(fileURL, height, width) {
        ...
        image.onload = () => {
            ...
            const imageData = Array.from(ctx.getImageData(0, 0, c1c.canvas.width, c1c.canvas.height).data).map((x) => x);
            setC1d(imageData);
            setC2d(imageData);
            setC3d(imageData);
            setC4d(imageData);
    ...
    return (
        <Box style={{backgroundColor:'#47515d', borderRadius:20}}>
            <Stack spacing={3} margin='1.5%' direction='row'>
                <Button onClick={loadButtonHandler} size='large' variant="contained" color='error'>
                    Load Image
                </Button>
                <Button onClick={(e) => {console.log(c1d);}} size='large' variant="contained" color='error'>
                    Save Results
                </Button>
